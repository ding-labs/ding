...
func TestDiscordNotifier_Drain_waitsForInFlightDelivery(t *testing.T) {
	delivered := make(chan struct{}, 1)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestDiscordNotifier_Drain_respectsTimeout(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		time.Sleep(10 * time.Second)
	}))
	defer srv.Close()

	n := notifier.NewDiscordNotifier(srv.URL, 1, 1*time.Millisecond, nil)
	if err := n.Send(makeRunAlert()); err != nil {
		t.Fatal(err)
	}
