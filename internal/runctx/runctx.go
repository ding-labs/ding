...
func (c *Context) detect() {
	...
	case os.Getenv("RAY_JOB_ID") != "":
		c.Runner = "ray"
		c.RunID = os.Getenv("RAY_JOB_ID")
	...
	case os.Getenv("KUBERNETES_SERVICE_HOST") != "":
		...
		c.Runner = "kubernetes"
		c.RunID = os.Getenv("POD_UID")
		if c.RunID == "" {
			c.RunID = os.Getenv("POD_NAME")
		}
		setIf(c.Labels, "namespace", os.Getenv("POD_NAMESPACE"))
		setIf(c.Labels, "pod", os.Getenv("POD_NAME"))
		setIf(c.Labels, "node", os.Getenv("NODE_NAME"))
		setIf(c.Labels, "job_name", os.Getenv("JOB_NAME"))
	}
}
